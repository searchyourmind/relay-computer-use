...
The original console is the primary demo and is included in this repository
alongside the Python backend and deployment configuration. Railway hosts two
Docker services: the console, Playwright/Chromium and synthetic bank in one,
and private Ollama inference in the other. This demo has no database: visitor
workspaces and run ownership live in the console process's memory and reset on
restart. Running the project does not require Lovable.
